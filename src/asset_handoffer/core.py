...
class Config:
    ...
    def load(config_file: Path) -> "Config":
        ...
        if isinstance(workspace, dict):
            ...
            root, inbox_name, repo_name, failed_name = (
                workspace,
                "inbox",
                ".repo",
                "failed",
    ...
    def _validate(self):
        ...
        if not has_rules and not has_pattern:
            raise ConfigError(
                m.t("config.missing_field", field="naming.rules or naming.pattern")
            )
    ...
    def git_user_email(self) -> str:
        return (
            self.data.get("git", {})
            .get("user", {})
            .get("email", "asset-handoffer@local")
        )
...
'''
    ...
    def create(
        ...
        content = content.replace('branch: "main"', f'branch: "{branch}"')

        if token:
            content = content.replace(
                '# token: ""  # 可选：Git 访问令牌（或设置环境变量 GIT_TOKEN）',
                f'token: "{token}"',
            )
...
def compute_target_path(
    parsed: dict, default_template: str, asset_root: str, repo_base: Path
) -> Path:
...
def process_file(
    file_path: Path, config: Config, output: Callable[[str], None] = print
) -> ProcessResult:
    ...
    try:
        ...
        if file_path.parent.exists():
            ...
            return ProcessResult(
                False, m.t("process.git_failed_moved_back", error=str(e))
            )
        else:
...
def process_batch(
    files: list[Path], config: Config, output: Callable[[str], None] = print
) -> tuple[int, int]:
    m = config.messages
    success = failed = 0
